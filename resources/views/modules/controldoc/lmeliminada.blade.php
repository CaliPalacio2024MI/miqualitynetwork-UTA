    ...
    <div class="p-6 bg-white rounded-2xl shadow-md h-full flex flex-col overflow-y-auto">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center"> <!-- Contenedor flex para alinear el texto y la imagen -->
                <img src="/images/listaeliminados.png" alt="Lista Maestra" class="w-12 h-12"> <!-- Imagen alineada a la derecha del texto -->    
                <h2 class="text-2xl font-bold text-gray-900">LMCIDEIYE</h2>
            </div>
        </div>

        <p class="text-lg text-gray-600 mb-4">Lista Maestra Control de Información Documentada Eliminada Interna y Externa.</p>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm rounded-lg">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-2 text-left sticky top-0 bg-[#092034] text-[#FFF] font-semibold">Razón de eliminación</th>
                            <th scope="col" class="px-4 py-2 text-left sticky top-0 bg-[#092034] text-[#FFF] font-semibold">Fecha de eliminación</th>
                            <th scope="col" class="px-4 py-2 text-left sticky top-0 bg-[#092034] text-[#FFF] font-semibold">Responsable(s) de eliminación</th>
                        </tr>
                    </thead>
                    <tbody id="table-body" class="divide-y divide-gray-100">
                        @foreach ($archivos as $archivo)
                            <tr class="odd:bg-gray-50 even:bg-gray-100 hover:bg-gray-200">
                                ...
                                <td class="px-4 py-3 text-gray-900">{{ $archivo->fechasubida }}</td>
                                <td class="px-4 py-3 text-gray-900">{{ $archivo->edicion }}</td>
                                <td class="px-4 py-3 text-gray-900">{{ $archivo->razon_eliminacion }}</td>
                                <td class="px-4 py-3 text-gray-900">{{ $archivo->fecha_eliminacion }}</td>
                                <td class="px-4 py-3 text-gray-900">{{ $archivo->responsable_eliminacion }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
