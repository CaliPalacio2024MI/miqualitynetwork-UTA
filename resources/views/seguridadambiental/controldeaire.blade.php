...
@section('title', 'Control de Aire')

@section('content')
@vite(['resources/css/CapturaDeEnergetico.css'])
<div class="max-w-md p-6 mx-auto">
    <h1 class="font-bold text-center" style="color: #011a32; font-size: 1.875rem; line-height: 2.25rem; margin-bottom: 0.5rem;">Control de Aire</h1>
    <p class="text-center" style="color: #011a32; font-size: 1.25rem; line-height: 1.75rem; margin-bottom: 1.5rem;">Captura del consumo de aire</p>
    
    @if(session('success'))
        <div class="mb-4 alert alert-success">{{ session('success') }}</div>
    @endif
    
    <form id="form-aire" method="POST" action="{{ route('captura.aire.store') }}" style="background-color: #f8fafc; padding: 1.0rem; border-radius: 0.5rem; border: 1px solid #e2e8f0;">
        @csrf
        ...
        <div class="mb-4">
            ...
            <select name="energetico_id" id="energetico_id" style="width: 100%; padding: 0.4rem; border: 1px solid #cbd5e0; border-radius: 0.25rem; background-color: white;">
                ...
                @if(isset($energeticos))
                    @foreach($energeticos->where('modulo', 'aire') as $energetico)
        ...
        <!-- Consumo -->
        <div class="mb-4">
            <label style="display: block; font-weight: 600; color: #011a32; margin-bottom: 0.5rem;">Cantidad (Litros):</label>
        ...
        <!-- Costo Promedio -->
        <div class="mb-4">
            <label style="display: block; font-weight: 600; color: #011a32; margin-bottom: 0.5rem;">Costo por litro:</label>
...
<script src="{{ asset('js/aire.js') }}"></script>
@endsection