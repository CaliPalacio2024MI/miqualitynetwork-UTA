...
class Inocuidadcontroller extends Controller
{
    // Vista principal de la sección "Inocuidad"
    public function inocuidad()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_inocuidad) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'inocuidad')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadalimentaria.inocuidad', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadalimentaria.inocuidad', compact('carpeta', 'archivos'));
    }
