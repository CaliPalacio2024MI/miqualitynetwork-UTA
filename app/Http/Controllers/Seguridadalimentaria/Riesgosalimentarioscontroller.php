...
class Riesgosalimentarioscontroller extends Controller
{
    // Vista principal de la sección "Riesgos Alimentarios"
    public function riesgosalimentarios()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_riesgosalimentarios) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'riesgos_alimentarios')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadalimentaria.riesgosalimentarios', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadalimentaria.riesgosalimentarios', compact('carpeta', 'archivos'));
    }
