...
class Manipulaciondealimentoscontroller extends Controller
{
    // Vista principal de la sección "Manipulación de Alimentos"
    public function manipulaciondealimentos()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_manipulaciondealimentos) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'manipulacion_de_alimentos')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadalimentaria.manipulaciondealimentos', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadalimentaria.manipulaciondealimentos', compact('carpeta', 'archivos'));
    }
