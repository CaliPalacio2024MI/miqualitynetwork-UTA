...
class Cadenaalimentariacontroller extends Controller
{
    // Vista principal de la sección "Cadena Alimentaria"
    public function cadenaalimentaria()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_cadenaalimentaria) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'cadena_alimentaria')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadalimentaria.cadenaalimentaria', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadalimentaria.cadenaalimentaria', compact('carpeta', 'archivos'));
    }
