...
{
    // Vista principal de la sección "Revenue Reports"
    public function revenuereports()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_revenuereports) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'revenue_reports')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('calidad.revenuereports', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('calidad.revenuereports', compact('carpeta', 'archivos'));
    }
