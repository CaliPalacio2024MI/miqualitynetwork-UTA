...
class CafeteriadeanfitrionesController extends Controller
{
    // Vista principal de la sección "Cafetería de Anfitriones"
    public function cafeteriadeanfitriones()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_cafeteriadeanfitriones) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'cafeteria_de_anfitriones')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('calidad.cafeteriadeanfitriones', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('calidad.cafeteriadeanfitriones', compact('carpeta', 'archivos'));
    }
