...
class Controlplanesdeaccioncontroller extends Controller
{
    // Vista principal de la sección "Control de Planes de Acción"
    public function controlplanesdeaccion()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_controlplanesdeaccion) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'control_planes_de_accion')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('calidad.controlplanesdeaccion', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('calidad.controlplanesdeaccion', compact('carpeta', 'archivos'));
    }
