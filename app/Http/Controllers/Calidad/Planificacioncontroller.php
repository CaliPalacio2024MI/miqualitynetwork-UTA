...
{
    // Vista principal de la sección "Planificación"
    public function planificacion()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_planificacion) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'planificacion')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('calidad.planificacion', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('calidad.planificacion', compact('carpeta', 'archivos'));
    }
