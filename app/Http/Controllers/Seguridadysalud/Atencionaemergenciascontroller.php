...
class Atencionaemergenciascontroller extends Controller
{
    // Vista principal de la sección "Atención a Emergencias"
    public function atencionaemergencias()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_atencionaemergencias) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'atencion_a_emergencias')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadysalud.atencionaemergencias', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadysalud.atencionaemergencias', compact('carpeta', 'archivos'));
    }
