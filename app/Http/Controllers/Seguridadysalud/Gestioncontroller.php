...
{
    // Vista principal de la sección "Gestión"
    public function gestion()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_gestion) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'gestion')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadysalud.gestion', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadysalud.gestion', compact('carpeta', 'archivos'));
    }
