...
class Identificacionycontrolriesgoscontroller extends Controller
{
    // Vista principal de la sección "Identificación y Control de Riesgos"
    public function identificacionycontrolderiesgos()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_identificacionycontrolderiesgos) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'identificacion_y_control_de_riesgos')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadysalud.identificacionycontrolderiesgos', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadysalud.identificacionycontrolderiesgos', compact('carpeta', 'archivos'));
    }
