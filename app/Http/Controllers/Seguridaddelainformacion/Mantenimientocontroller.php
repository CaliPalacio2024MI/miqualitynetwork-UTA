...
{
    // Vista principal de la sección "Mantenimiento"
    public function mantenimiento()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_mantenimiento) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'mantenimiento')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridaddelainformacion.mantenimiento', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridaddelainformacion.mantenimiento', compact('carpeta', 'archivos'));
    }
