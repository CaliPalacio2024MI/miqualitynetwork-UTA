...
{
    // Vista principal de la sección "BCP"
    public function bcp()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_bcp) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'bcp')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridaddelainformacion.bcp', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridaddelainformacion.bcp', compact('carpeta', 'archivos'));
    }
