...
{
    // Vista principal de la sección "DRP"
    public function drp()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_drp) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'drp')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridaddelainformacion.drp', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridaddelainformacion.drp', compact('carpeta', 'archivos'));
    }
