...
{
    // Vista principal de la sección "Controles"
    public function circulares()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_circulares) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'circulares')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridaddelainformacion.circulares', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridaddelainformacion.circulares', compact('carpeta', 'archivos'));
    }
