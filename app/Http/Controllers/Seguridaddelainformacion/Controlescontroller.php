...
class Controlescontroller extends Controller
{
    ...
    public function controles()
    {
        ...
        if (!$user->privilegios || !$user->privilegios->acceso_controles) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'controles')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridaddelainformacion.controles', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridaddelainformacion.controles', compact('carpeta', 'archivos'));
    }
