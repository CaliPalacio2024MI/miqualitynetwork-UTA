...
class Reportedeenergiacontroller extends Controller
{
    // Vista principal de la sección "Reporte de Energía"
    public function reportedeenergia()
    {
        $user = Auth::user();
        ...
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'reporte_de_energia')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadambiental.reportedeenergia', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadambiental.reportedeenergia', compact('carpeta', 'archivos'));
    }
