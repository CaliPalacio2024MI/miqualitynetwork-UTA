...
class Informaciondeenergiacontroller extends Controller
{
    // Vista principal de la sección "Información de Energía"
    public function informaciondeenergia()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_informaciondeenergia) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'informacion_de_energia')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadambiental.informaciondeenergia', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadambiental.informaciondeenergia', compact('carpeta', 'archivos'));
    }
