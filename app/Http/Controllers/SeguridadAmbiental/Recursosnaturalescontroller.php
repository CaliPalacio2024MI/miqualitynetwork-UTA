...
class Recursosnaturalescontroller extends Controller
{
    // Vista principal de la sección "Recursos Naturales"
    public function recursosnaturales()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_recursosnaturales) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'recursos_naturales')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadambiental.recursosnaturales', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadambiental.recursosnaturales', compact('carpeta', 'archivos'));
    }
