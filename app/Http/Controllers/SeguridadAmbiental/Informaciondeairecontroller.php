...
class Informaciondeairecontroller extends Controller
{
    // Vista principal de la sección "Información de Aire"
    public function informaciondeaire()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_informaciondeaire) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'informacion_de_aire')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadambiental.informaciondeaire', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadambiental.informaciondeaire', compact('carpeta', 'archivos'));
    }
