...
class informaciondeaguacontroller extends Controller
{
    // Vista principal de la sección "Información de Agua"
    public function informaciondeagua()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_informaciondeagua) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'informacion_de_agua')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadambiental.informaciondeagua', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadambiental.informaciondeagua', compact('carpeta', 'archivos'));
    }
