...
{
    // Vista principal de la sección "Comunidad"
    public function comunidad()
    {
        $user = Auth::user();

        // Validar acceso del usuario
        if (!$user->privilegios || !$user->privilegios->acceso_comunidad) {
            return view('layouts.dashboard');
        }

        // Obtener carpetas principales accesibles para el usuario
        $carpetas = $user->carpetasAccesibles()
            ->where('subseccion', 'comunidad')
            ->whereNull('parent_id') // Solo carpetas principales
            ->get();

        return view('seguridadambiental.comunidad', compact('carpetas'));
    }
    ...
    public function listarArchivos($carpeta_id)
    {
        ...
        return view('seguridadambiental.comunidad', compact('carpeta', 'archivos'));
    }
